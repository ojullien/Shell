...
createGroup () {
    ...
    if [ 0 -eq $iReturn ]; then
        success "Creating group $1."
    else
        error "ERROR: Can not create the group $1. code: $iReturn"
    fi
...
createUser () {
    ...
    iReturn=$?
    if [ 0 -eq $iReturn ]; then
        success "Creating user $1."
    else
        error "ERROR: Can not create the user $1. code: $iReturn"
    fi
...
createDirectory () {
    if [ $# -eq 0 ] || [ -z "$1" ]; then
        error "Usage: createDirectory <directory>"
        exit 1
    fi
    ...
    if [ 0 -eq $iReturn ]; then
        ...
        error "ERROR: Can not create the directory $1. code: $iReturn"
    fi
...
changeOwner () {
    ...
    iReturn=$?
    if [ 0 -eq $iReturn ]; then
        success "Changing owner/group and access right for $1."
    else
        error "ERROR: Can not change owner/group and access right for $1. code: $iReturn"
    fi
